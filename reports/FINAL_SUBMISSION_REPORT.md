...
오래된 대규모 C/C++ 저장소에서 위험 함수 호출만 나열하면 오탐과 중복이 많고, 전체 코드를 한 번에 AI 입력으로 보내기 어렵다. **BugBori(버그보리)** 는 위험 호출을 **검토 후보**로만 탐지한 뒤, 필요한 문맥과 원문 근거를 단계적으로 모아 검증하는 SAST 워크플로를 구현했다. 후보가 발견됐다는 사실만으로 실제 취약점이라고 확정하지 않는다.

이 도구의 목적은 취약 가능성을 찾는 것이다. 전체 스캔에서 583개 후보를 수집하고, 그중 높은 우선순위 원본 후보 11개를 10건으로 통합해 정밀 검토했다. 이 과정에서 1건은 취약 가능성 높음, 8건은 방어 근거로 낮음, 1건은 정보 부족으로 분류했다.
...
이 사례는 “위험 함수가 있다”는 단순 탐지 결과가 아니라, 명령행 입력 출처·함수 전달 경로·고정 배열 크기·선행 길이 검사의 부재를 함께 검토해 우선순위를 높인 결과다.
